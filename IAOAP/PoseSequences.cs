...
{
    /// <summary>
    /// 執行機器人姿態序列：依輪替優先順序(A→B→C→D)觸發單一區域動作，
    ...
    public static class PoseSequences
    {
        ...
        private static int priorityIndex = 0;

        public static readonly Dictionary<string, List<double[]>> Sequences = new Dictionary<string, List<double[]>>
        {
            ["1-D"] = new List<double[]> {
                new double[]{ -205, 0, 0, 0, 0, 0 },
                new double[]{ -205, 19.8, 0, 0, 0, 0 },
                new double[]{ -205,-23.2, 0, 0, 0, 0 },
            ...
            ["2-D"] = new List<double[]> {
                new double[]{ 120, 0, 0, 0, 0, 0 },
                new double[]{ -205, 0, 0, 0, 0, 0 },
                new double[]{ -205, -10.8, -16.1, -37.4, 0, 0 },
                new double[]{ -205, 15.3, -16.1, -37.4, 0, 0 },
                new double[]{ -205, -6.8, -16.1, -37.4, 0, 0 },
                new double[]{ 120, -6.8, -16.1, -37.4, 0, 0 }
            },
            ["2-A"] = new List<double[]> {
                new double[]{ 120, 0, 0, 0, 0, 0 },
                new double[]{ -205, 0, 0, 0, 0, 0 },
            ...
            ["3-D"] = new List<double[]> {
                new double[]{ -205, 0, 36.6, 0, 0, 0 },
                new double[]{ -205, 0, -14.7, 0, 0, 0 },
                new double[]{ -205, 70.3, -55, 0, 0, 0 },
                new double[]{ -205, 2.6, 18.2, 0, 0, 0 },
                new double[]{ -205, 2.6, 18.2, 36.5, 0, 0 },
                new double[]{ -205, 2.6, 18.2, -30.9, 0, 0 }
            },
            ["3-A"] = new List<double[]> {
            ...
            ["more-than-3"] = new List<double[]> {
                new double[]{ -205, 34, -49.7, 0, 0, 0 },
                new double[]{ 120, 34, -49.7, 0, 0, 0 },
        ...
        {
            double j1Offset = 45.0;
            foreach (var joints in seqList)
            {
                var adjustedJoints = (double[])joints.Clone();
                adjustedJoints[0] += j1Offset;
                var jointStr = string.Join(",", adjustedJoints);
                Console.WriteLine($"ExecuteZone: 呼叫 ptp_axis, joints=[{jointStr}]");
                MotionDoneEvent.Reset();
                int ret = HRobot.ptp_axis(handle, 0, adjustedJoints);
                if (ret != 0)
                {
        ...
        private static void ExecuteSequences(List<Region> regions, int handle)
        {
            ...
            string[] allZoneIds = { "1", "2", "3", "4" };
            var rotatedZoneIds = allZoneIds.Skip(priorityIndex).Concat(allZoneIds.Take(priorityIndex)).ToList();
            priorityIndex = (priorityIndex + 1) % 4;

            foreach (var id in rotatedZoneIds)
            {
                ...
                string SoundPC_IP = "192.168.1.154";
                int SoundPC_port = 8082;
                string LightPC_IP = "192.168.1.99";
                int LightPC_port = 9897;

                var messageMap = new Dictionary<(int, string), string>
                {
                    [(1, "A")] = "C",
                    [(1, "B")] = "D",
                    [(1, "C")] = "E",
                ...
                string msg = messageMap.TryGetValue((count, zone), out var val) ? val : "O";
                UdpSender.SendMessage(SoundPC_IP, SoundPC_port, msg);
                UdpSender.SendMessage(LightPC_IP, LightPC_port, msg);
                ...
                {
                    Console.WriteLine($"ExecuteSequences: 無對應的動作序列 key={key}");
                }
